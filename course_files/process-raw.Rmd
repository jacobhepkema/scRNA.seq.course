...
At times it may be useful to manually inspect files - for example to check the metadata in headers that the files 
are from the correct sample. Commands 'less' and 'more' can be used to inspect any text files from the command line.
By "pipe-ing" the output of samtools view into these commands using '|' we check each of these file types without having to save multiple copies of each file.
